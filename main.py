...
LANG = json.loads(open(PATH + 'lang/' + CONFIG['Lang'] + '.json', 'rb').read())  # 加载语言文件
...
def process_command(bot, update):  # 处理指令
    ...
    if command[0] == 'start':
        bot.send_message(chat_id=update.message.chat_id,
                         text=LANG['start'])
        return
    elif command[0] == 'adminlogin':
        admins = [CONFIG['manager6'],CONFIG['manager7'],CONFIG['manager8']]
        if int(update.message.chat_id) == CONFIG['Admin']:
            bot.send_message(chat_id=update.message.chat_id,
                         text='你已经是我的主人了，无需再次申请成为我的主人。')
        elif CONFIG['Admin'] in admins :
            bot.send_message(chat_id=update.message.chat_id,
                         text='已经有控制我的主人啦，请先logout退出控制模式。')
        elif int(update.message.from_user.id) in admins and int(update.message.chat_id) in admins :
            CONFIG['Admin'] = int(update.message.chat_id)
            bot.send_message(chat_id=update.message.chat_id,
                         text='你已经成功控制了该bot管理员模式，所有消息会发送给你，原管理员已经无法收到消息。')
        return
    elif command[0] == 'logout':
        admins = [CONFIG['Admin'],CONFIG['manager6'],CONFIG['manager7'],CONFIG['manager8'],admin0]
        if int(update.message.from_user.id) in admins and int(update.message.chat_id) in admins :
            CONFIG['Admin'] = int(admin0)
            bot.send_message(chat_id=update.message.chat_id,
                         text='管理员已退出，已恢复正常模式')
        return
        ...
        bot.send_message(chat_id=update.message.chat_id,
                         ...
                         + Version_Code
                         + '\nhttps://github.com/610312586/Telegram-chatbot'
                         )
        return
    elif command[0] == 'clearadmin': # 管理者取消管理员
        admins = [CONFIG['manager6'],CONFIG['manager7'],CONFIG['manager8']]
        if int(update.message.from_user.id) in admins :
            CONFIG['Admin'] = 0
            save_config()
            bot.send_message(chat_id=update.message.chat_id,
